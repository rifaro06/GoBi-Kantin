...
// Helper untuk membuat Link WhatsApp beserta Pesan Otomatis
// Helper untuk membuat Link WhatsApp dengan 5 Pesan Otomatis & Catatan Detail
const getWaAdminUrl = (order) => {
  if (!order || !order.customer_phone) return '#';

  // 1. Format Nomor HP ke '628...'
  ...
  const totalAmount = formatRupiah(order.total_amount);

  // Formatting Rincian Item + Varian + Catatan
  const itemsText = order.items?.length > 0
    ? order.items.map(i => {
      let itemLine = `• ${i.qty}x ${i.product?.name || i.name || 'Menu'}`;
      if (i.variant) itemLine += ` (Varian: ${i.variant})`;
      if (i.note) itemLine += `\n   _Catatan: "${i.note}"_`;
      return itemLine;
    }).join('\n')
    : '• Detail pesanan';
  ...
  // 3. Logika 5 Kondisi Pesan WhatsApp
  const isQrisUnpaid = order.payment_method === 'QRIS' && order.payment_status !== 'PAID';

  if (isQrisUnpaid) {
    // 1. QRIS BELUM BAYAR (Konfirmasi Pembayaran)
    message = `Halo Kak *${customerName}*, kami dari *GoBi Kantin* 👋\n\n` +
      `Mengenai pesanan *${orderNumber}* (${className}):\n` +
      `${itemsText}\n\n` +
      `*Total Tagihan:* Rp ${totalAmount}\n` +
      `*Metode Pembayaran:* QRIS (Belum Konfirmasi)\n\n` +
      `Mohon kirimkan *foto / screenshot bukti pembayaran QRIS* di chat ini ya Kak, agar pesanan dapat langsung kami proses. Terima kasih! 🙏`;

  } else if (order.status === 'PENDING') {
    // 2. PESANAN BARU / PENDING
    message = `Halo Kak *${customerName}*, terima kasih sudah memesan di *GoBi Kantin* 👋\n\n` +
      `Pesanan *${orderNumber}* (${className}) sudah kami terima:\n` +
      `${itemsText}\n\n` +
      `*Total Tagihan:* Rp ${totalAmount} (${order.payment_method || 'CASH'})\n` +
      `*Status:* Pesanan Masuk (Menunggu Antrean)\n\n` +
      `Pesanan Kakak sedang masuk antrean dapur. Mohon ditunggu ya Kak! 🙏`;

  } else if (order.status === 'DIPROSES') {
    // 3. DIMASAK / DIPROSES
    message = `Halo Kak *${customerName}*, update pesanan *${orderNumber}* 🍳\n\n` +
      `Detail pesanan:\n` +
      `${itemsText}\n\n` +
      `*Status:* Sedang Dimasak / Disiapkan\n\n` +
      `Pesanan Kakak saat ini sedang dimasak di dapur Kantin. Kami beritahukan lagi jika makanan sudah siap diantar ke kelas *${className}*! 😋`;

  } else if (order.status === 'DIANTAR') {
    // 4. DIANTAR
    message = `Halo Kak *${customerName}*, pesanan *${orderNumber}* sedang diantar! 🛵💨\n\n` +
      `Detail pesanan:\n` +
      `${itemsText}\n\n` +
      `*Status:* Dalam Pengantaran\n` +
      `*Tujuan:* Kelas ${className}\n\n` +
      `Kurir kami sedang menuju ke kelas Kakak. Mohon bersiap menerima pesanan ya Kak! Terima kasih 🙏`;

  } else if (order.status === 'SELESAI') {
    // 5. SELESAI
    message = `Halo Kak *${customerName}*, pesanan *${orderNumber}* telah selesai! ✅\n\n` +
      `Rincian pesanan:\n` +
      `${itemsText}\n\n` +
      `*Status:* Selesai / Terkirim\n\n` +
      `Terima kasih sudah berbelanja di *GoBi Kantin*! Selamat menikmati hidangannya Kak! ❤️`;

  } else {
    // Fallback Default
    message = `Halo Kak *${customerName}*, info pesanan *${orderNumber}* (${className}):\n\n${itemsText}\n\nTotal: Rp ${totalAmount}`;
  }
...
export default function OrdersAdmin() {
  ...
  const handleUpdateStatus = async (orderId, currentStatus) => {
    ...
    try {
      await axios.patch(`/admin/orders/${orderId}/status`, { status: nextStatus });
      fetchOrders();
  ...
  const handleLunas = async (orderId) => {
    try {
      await axios.patch(`/admin/orders/${orderId}/status`, { payment_status: 'PAID' });
      fetchOrders();
                ...
                <div>
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-emerald-700 font-bold">
                      <MapPin className="w-4 h-4 text-emerald-500 shrink-0" /> {order.class_room?.name || 'Kelas'}
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl space-y-3 border border-slate-100">
                  ...
                  <div className="flex items-center justify-between text-slate-700">
                    ...
                    {/* Tombol Chat WA Dinamis */}
                    <a
                      href={getWaAdminUrl(selectedOrder)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[10px] bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-md font-bold hover:bg-emerald-200 transition-colors"
                    >
                      Chat WA
                    </a>
                  </div>
                  <div className="flex items-center gap-3 text-slate-700">
                    ...
                    <span className="font-bold">{selectedOrder.class_room?.name || 'Kelas'}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 text-sm">
                  {selectedOrder.items?.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-start border-b border-slate-200 last:border-0 pb-2 last:pb-0">
                      <div>
                        <span className="font-bold text-slate-700">{item.qty}x {item.product?.name || item.name || 'Menu Telah Dihapus'}</span>
                        {item.variant && <p className="text-[10px] text-emerald-600 font-semibold mt-0.5">Varian: {item.variant}</p>}
                        {item.note && <p className="text-[10px] text-slate-500 italic mt-0.5">Catatan: {item.note}</p>}
                      </div>
                      <span className="font-bold text-slate-700">Rp {formatRupiah((item.price || 0) * (item.qty || 1))}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-black text-slate-800 text-base">Rp {formatRupiah(selectedOrder.total_amount)}</span>
                  </div>
                  ...
                  {selectedOrder.payment_method === 'CASH' && (
                    <>
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="font-bold text-emerald-600">Rp {formatRupiah(selectedOrder.cash_amount)}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-200">
                        <span className="text-slate-600 flex items-center gap-2"><Wallet className="w-4 h-4" /> Kembalian</span>
                        <span className="font-black text-rose-500">Rp {formatRupiah(selectedOrder.change_amount)}</span>
                      </div>
                    </>
                  )}
                </div>
