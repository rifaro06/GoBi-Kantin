...
export default function Checkout() {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      try {
        ...
        const isOpen =
          currentSettings.kantin_open == 1 ||
          currentSettings.kantin_open === '1' ||
          currentSettings.kantin_open === true;
  ...
  const totalHandlingFee = cartItems.reduce((sum, item) => {
    const itemHandlingFee = Number(item.handling_fee) || 0;
    return sum + itemHandlingFee * item.qty;
  }, 0);
  const computedTotal = subtotalProducts + deliveryFee + totalHandlingFee;
        ...
        {cartItems.length === 0 ? (
          <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center space-y-4 shadow-xs my-8 max-w-md mx-auto">
            ...
            <button
              type="button"
              onClick={() => navigate('/')}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm px-6 py-3 rounded-xl transition-all shadow-md cursor-pointer"
            >
              Kembali Pilih Menu
            </button>
          </div>
              ...
              <div className="bg-white p-5 rounded-3xl shadow-xs border border-slate-200 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    {errors.class_room_id && (
                      <p className="text-[11px] font-bold text-rose-500 mt-1.5 pl-1">• {errors.class_room_id}</p>
                    )}
                  </div>
                </div>
              </div>
                        ...
                        <div className="flex gap-3 items-center">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex items-center bg-slate-100 rounded-xl p-1 border border-slate-200">
                              <button
                                type="button"
                                onClick={() => handleSafeDecreaseQty(itemId)}
                                className="w-6 h-6 bg-white rounded-lg flex items-center justify-center text-slate-700 shadow-xs hover:bg-slate-50 cursor-pointer"
                              >
                              ...
                              <button
                                type="button"
                                onClick={() => handleSafeIncreaseQty(itemId)}
                                className="w-6 h-6 bg-white rounded-lg flex items-center justify-center text-slate-700 shadow-xs hover:bg-slate-50 cursor-pointer"
                              >
            ...
            <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
              ...
              <div className="bg-white p-5 rounded-3xl shadow-xs border border-slate-200 space-y-3">
                ...
                <div className="space-y-2 text-sm text-slate-500 pt-1">
                  ...
                  {totalHandlingFee > 0 && (
                    <div className="flex justify-between items-center">
                      <span>Biaya Kemasan / Penanganan</span>
                      <span className="font-bold text-slate-700">Rp {totalHandlingFee.toLocaleString('id-ID')}</span>
                    </div>
                  )}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {showInfo && (
                        <div className="absolute bottom-6 left-0 w-64 p-3 bg-slate-800 text-white text-[10px] rounded-2xl shadow-xl z-20">
                          Biaya antar dihitung otomatis berdasarkan kategori produk, jumlah makanan, subtotal
                          snack/minuman, serta total item.
                        </div>
                      )}
                    </div>
                ...
                {paymentMethod === 'CASH' && (
                  <div className="mt-4 pt-4 border-t border-slate-100 space-y-3 bg-slate-50 p-4 rounded-2xl">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700">
                      ...
                      <span>Perhitungan Kembalian Tunai</span>
                    </div>
                    <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      <button
                        type="button"
                        onClick={() => {
                          setCashPaid(computedTotal.toString());
                          if (errors.cashPaid) setErrors((prev) => ({ ...prev, cashPaid: null }));
                        }}
                        className="text-[11px] bg-white border border-slate-200 font-bold px-2.5 py-1 rounded-lg hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer"
                      >
                        Uang Pas
                      </button>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold py-4 rounded-2xl shadow-lg shadow-emerald-600/20 transition-all flex items-center justify-center gap-2 text-base disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
